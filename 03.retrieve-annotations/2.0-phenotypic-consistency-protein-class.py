...
retrieval_label = "protein_class"
multi_label_col = "Metadata_protein_class_list"
annotation_col = "Metadata_protein_class"
...
orf_annotation_df = (
    ...
    .assign(
        col=lambda x: x[f"{annotation_col}"].str.split("|")
    )
    .rename(columns={"col": f"{multi_label_col}"})
    .explode(f"{multi_label_col}")
    .query(f"{multi_label_col} in @protein_class_list")
